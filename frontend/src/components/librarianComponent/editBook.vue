    ...
    <div class="row justify-content-center">
      <div class="col-12">
        <div class="card shadow-sm border">
          <div class="card-body">
            <h2 class="card-title mb-4 text-center">Edit Book</h2>
            <form @submit.prevent="updateBook" class="needs-validation" novalidate>
              <div class="row text-start p-2">
                ...
                <div class="col-md-6 mb-3">
                  ...
                  <input type="file" class="form-control" id="bookImage" ref="imageFile" accept="image/*">
                </div>

                <div class="col-md-6 mb-3">
                  <label for="bookPDF" class="form-label">Upload Book PDF</label>
                  <input type="file" class="form-control" id="bookPDF" ref="pdfFile" accept=".pdf">
                </div>
...
export default {
  ...
  methods: {
    ...
    async updateBook() {
      ...
      formData.append('id', this.book_data.id);
      ...
      try {
        const response = await fetch(`http://127.0.0.1:5000/api/book/${this.book_data.id}`, {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          this.serverMessage = "Book updated successfully";
          ...
          this.serverErrorMessage = error.message || "Failed to update book";
  ...
  async mounted() {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5000/api/book/${this.$route.params.productId}` ,
        {
          headers: {
      ...
      this.book_data.publish_date = new Date(this.book_data.publish_date).toISOString().split('T')[0];
      // Fetch sections for the select field
      const sectionsResponse = await fetch('http://127.0.0.1:5000/api/section' ,
        {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
